...
class MainActivity : ComponentActivity() {
    ...
    private val pickWidgetLauncher = registerForActivityResult(
        ...
        if (result.resultCode == RESULT_OK && appWidgetId != AppWidgetManager.INVALID_APPWIDGET_ID) {
            try {
                ...
                showWidgetError("Não foi possível configurar este widget.")
            } catch (e: Exception) {
                Log.w(TAG, "Erro inesperado ao processar widget escolhido", e)
                cancelPendingWidget(appWidgetId)
                showWidgetError("Não foi possível adicionar este widget.")
            }
        } else {
    ...
    private fun startAddWidgetFlow() {
        ...
        if (appWidgetId == AppWidgetManager.INVALID_APPWIDGET_ID) {
            showWidgetError("Não foi possível iniciar a criação do widget.")
            return
        }
        ...
        try {
            ...
            cancelPendingWidget(appWidgetId)
            showWidgetError("Nenhum seletor de widgets disponível neste aparelho.")
        } catch (e: Exception) {
            Log.w(TAG, "Erro inesperado ao abrir o seletor de widgets", e)
            cancelPendingWidget(appWidgetId)
            showWidgetError("Não foi possível abrir o seletor de widgets.")
        }
    }
    ...
    private fun finalizeWidget(appWidgetId: Int) {
        try {
            ...
            cancelPendingWidget(appWidgetId)
            showWidgetError("Não foi possível salvar o widget.")
        }
    }
    ...
    private fun launchApp(app: AppInfo) {
        try {
            ...
            try {
                ...
                showWidgetError("Não foi possível abrir ${app.label}.")
            }
        }
    }
