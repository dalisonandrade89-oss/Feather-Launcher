...
class WidgetHostManager(private val context: Context) {
    ...
    fun loadAvailableWidgets(cellSizeDp: Int): List<WidgetProviderOption> {
        ...
        return providers.mapNotNull { info ->
            try {
                ...
                val appLabel = loadAppLabel(pm, info.provider.packageName)
                val description = loadDescriptionIfAvailable(info)
            ...
            // FIX #7: agrupado por app primeiro, e só depois por nome do
            // widget — assim, apps com vários widgets (ex.: 2 tamanhos de
            // um mesmo widget de clima) ficam sempre em sequência na lista,
            // em vez de espalhados em ordem alfabética global.
        }.sortedWith(compareBy({ it.appLabel.lowercase() }, { it.label.lowercase() }))
    }

    private fun loadAppLabel(pm: android.content.pm.PackageManager, packageName: String): String = try {
    ...
    /**
     * Descrição curta do widget (ex.: "Mostra a previsão dos próximos 5
     * dias"), quando o provider a declarou — só disponível a partir do
     * Android 12 (API 31). Em versões anteriores, retorna null; quem
     * exibe a lista já mostra o tamanho sugerido como diferenciador
     * alternativo nesse caso (ver WidgetProviderRow).
     */
    private fun loadDescriptionIfAvailable(info: AppWidgetProviderInfo): String? {
        if (android.os.Build.VERSION.SDK_INT < 31) return null
        return try {
            info.loadDescription(context)?.toString()?.takeIf { it.isNotBlank() }
        } catch (e: Exception) {
            null
        }
    }
